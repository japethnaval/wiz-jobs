  ...
  return (
    <div
      className={mergeClass(
        "relative z-10 w-full overflow-visible pb-0 md:pb-12 lg:pb-16 h-full",
        className,
      )}
    >
      <ScaleHoverMotion
        className="group mx-auto block max-w-full h-full"
        style={boxStyle}
      >
        <article className="relative z-10 isolate aspect-square w-full max-w-full shrink-0">
          <div className="relative flex h-full w-full items-center justify-center">
            ...
            <div
              className={mergeClass(
                "md:min-h-[380px] md:min-w-[380px] relative z-10 h-full w-full overflow-hidden rounded-full border-[5px] border-[#49FBDF] shadow-sm transition-shadow duration-300 ease-out sm:border-[6px]",
            ...
            >
              <div className="absolute inset-1.5 z-20 flex flex-col items-center justify-center rounded-full bg-white px-4 text-center sm:inset-2 sm:px-4 lg:py-4">
                ...
                <div className="px-2 md:px-[.4rem] mt-2 max-w-[min(100%,15.5rem)] space-y-2 text-pretty text-sm leading-snug text-black max-[450px]:text-[0.8rem] sm:mt-4 sm:max-w-[18rem] sm:space-y-3 sm:leading-relaxed md:text-sm">
                  {children}
                </div>
              </div>
            </div>
